...
gofresh's own root pin is that call, and stipulator's is too. gomutant
and pew still carry byte-identical copies of the former checker in
their `shortgates_test.go`; each collapses to the call at its bump to
the gofresh release that carries the package.

Lands: the consumers' bumps — cross-tool train chunks 156 (gomutant)
and 157 (pew), gofresh docs/plans/cross-tool-train.md.
