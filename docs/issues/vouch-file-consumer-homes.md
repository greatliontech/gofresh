...
`Lands: each consumer's gofresh bump past the release carrying
REQ-vouch-input's file channel — gomutant at cross-tool train chunk
163, pew and stipulator at their next gofresh bump`
...
- pew reads its store's `vouches` (REQ-pew-vouch-source, the store root
  ...
  duplicated reader.
- gomutant's `--vouch` flags extend the file's set at its bump with no
  code change; the fleet-sweep gatherer's mirrored flag list can then go
  once the repository file carries the standing set.
