...
files (85 sites) and forbids `t.Parallel()`; `SetMemoRoot` /
`DisableMemos` are package globals; `ExplainDynamicState` holds a
process-global mutex. Landing it means a per-test audit of shared
state, then measuring the wall under the CI core count.
