...
one index in final position; for any other root, selector steps over
...
while the holder's own storage stays its own. Range bindings link as
sharing regardless of element kind — fail-closed. Reach-free values
stay unlinked as independent storage. While any
