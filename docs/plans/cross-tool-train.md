...
- [ ] 280. stipulator: the self-host warm path (chartered at 272's
      close) — every engine-building witness of the tool's own corpus
      records gofresh's memo files under the harness cache home as
      uncovered runtime inputs (dated from 2026-09-06 in the store) and
      never serves, so a plain warm check executes the race invocation's
      long packages every run (51.3s at 155's verdict → 45 minutes);
      identify the 2026-09-06 change, place the exclusion (the memo is a
      cache, never a record — gofresh REQ-closure-observability-memo) or
      the per-test memo root, re-record once, measure the plain warm
      check; the resolution cache's empty race selection
      (resolution-cache-empty-for-the-checks-selection) rides if its
      diagnosis is the same seam. Directly after 272.
...
- [ ] 275. pew: the bump behind gofresh 265, 266, and 274 (264 C1, A1,
