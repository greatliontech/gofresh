...
**Proposed partition.**
1. *Pure tier — keep on the default path:* the verdict ladder and guard comparison (`gofresh.go:928-1005`, `FuzzDecideSound`, `structural_test.go`), `toolchainSkew`'s pure core, receiver-name derivation, explain ordering, `guidance/`, `internal/buildflags`, `gotool` (the former `internal/processenv` and `internal/gotool`), `internal/puredirective`, `closure/effectscan_reference_test.go`, the `cacheisolation_test.go` files, `guard`'s pure parse tier. ~40–60 of ~640 tests; single-digit seconds.
