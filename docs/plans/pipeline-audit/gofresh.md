...
**Hygiene defect the partition must fix.** Plain `go test ./...` writes into the user's real cache: `effectScanScope()` never returns empty (`closure/effectmemo.go:39-46`), so `storeEffectScan` (`closure/maximal.go:1066`) persists under `os.UserCacheDir()/gofresh/effectscan/` for every version-pinned package the suite touches; only two tests redirect the root (`view_test.go:3201-3241`). Suite timing is cache-state dependent; a cold-cache CI run is not the same workload as a warm dev run.
