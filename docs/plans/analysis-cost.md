...
- [ ] 3. Add batched observed checking: one bracket pair, one shared runtime-input observation window, and one shared drift analysis for a caller-supplied recording set, with a property test pinning batch↔single verdict equivalence; share one bracket pair in observed-refined capture the same way.
- [ ] 4. Batch observability analysis like refinement: group subjects by package, share the loaded program and attributed-RTA masks, and cache per-file maximal effect scans within a Hasher, pinned against REQ-closure-observability-batch-equivalence by a property test covering the shared-hasher prime→refine→observe sequence.
