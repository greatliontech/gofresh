...
// module variables whose source audit discharges the shared-dynamic-
// state marks — one key per (package NUL variable key), naming the
// audited versions. Every audit is a property of the listed versions'
// SOURCE, never of the module name: no other version inherits the
// proof — an unaudited version, later or earlier, refuses fail-closed
// until its source is audited — and a mutable-local checkout keeps
// every mark. One listing shape with the toolchain and harness audits
// (internal/auditset). Grows only by source audit
// (REQ-closure-shared-dynamic-state).
var auditedSourceListing = auditset.VersionListing{
	"golang.org/x/sys/unix\x00golang.org/x/sys/unix.mapper": {
...
func generatedProtoHeader(text string) string {
	...
	for _, generator := range []string{"protoc-gen-go-grpc", "protoc-gen-go"} {
		...
		// The generator name is a whole token — the same bound the
		// linkname directive match keeps (internal/auditset); the
		// longer name is tried first, so a shorter one bounded by the
		// longer's continuation is refused, never re-keyed; a bare
		// marker with nothing after the name is no header.
		rest, ok := auditset.BoundedToken(text, marker, " \t.")
		if !ok {
...
func composeDynamicState(meta []closure.GraphPackage, facts map[string][]dynamicStateFact, viewPackage string, vouches map[string]bool, singleSubject bool) *viewDynamicState {
	...
	auditedSourceOut := func(pkgPath, key string) bool {
		return pinnedPkg[pkgPath] && auditedSourceListing.Listed(pkgPath+"\x00"+key, pinOf[pkgPath])
	}
